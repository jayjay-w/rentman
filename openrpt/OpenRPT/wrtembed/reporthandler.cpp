...
void ReportHandler::dbConnect() {
	...
	if(dbConnected) {
		...
		login newdlg(0, "", true);
		newdlg.set(params);
		if (QMessageBox::question(0, "Open SQLiteDB", "Would you like to open an SQLite Database?", QMessageBox::Yes, QMessageBox::No) == QMessageBox::No) {
			if(newdlg.exec() == QDialog::Accepted) {
				OpenRPT::databaseURL = newdlg._databaseURL;
			}
		} else { //question messagebox answered Yes
			QString sqlFileName = QFileDialog::getOpenFileName(0, "Select SQLite database file");
